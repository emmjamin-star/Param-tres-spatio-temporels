...
if st.button("Lancer le calcul des paramètres spatio-temporaux"):
    try:
      ...
      if "LTOE" in labels:
          ...
          ltoe_valid_cycles = [
            (start, end) for start, end in zip(ltoe_cycle_start_indices, ltoe_cycle_end_indices)
            if (end - start) >= min_ltoe_cycle_duration
          ]
      ...
      if "RTOE" in labels:
          ...
          rtoe_valid_cycles = [
            (start, end) for start, end in zip(rtoe_cycle_start_indices, rtoe_cycle_end_indices)
            if (end - start) >= min_rtoe_cycle_duration
          ]
